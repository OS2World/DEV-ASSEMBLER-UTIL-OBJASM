...
** Local Prototypes
*/
char *seg_name( int, unsigned int * );
char *grp_name( int );
    ...
    int             index;
{
    ...
    switch( mode ) {
        case SEGMENT:   return( seg_name(index,&dummy) );       break;
        case GROUP:     return( grp_name(index) );              break;
        case LOCATION:  return( cseg_name );                    break;
    ...
    if ( dat_node && dat_node != data_tree ) {
        ...
        if ( dat_rec->seg_idx == index 
            ...
            */
            ...
            fix_search.offset     = (int)(disp - (unsigned long)dat_rec->offset);
            fix_rec = (FIX_T *)find( (char *)&fix_search, fix_tree,
                                                      fix_compare, &fix_node );
            if ( !fix_rec ) {
