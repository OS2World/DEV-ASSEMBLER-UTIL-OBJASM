...
printf("FIXUP:%04X:%08lX:%04X displacement=%08lX\n",
    ...
    insert( (char *)fixup_rec, fix_tree, fix_compare );
}
    ...
    int             extension;
{
    ...
    if ( pbit == 0 ) {
        displacement = (unsigned long)get_int();
        length += 2;
        if ( extended ) {
            displacement += (unsigned long)get_byte() << 16;
            displacement += (unsigned long)get_byte() << 24;
            length += 2;
        }
    } else {
        displacement = 0L;
...
void fixupp( length, extension )
    ...
    int             extension;
{
    unsigned char   type;           /* First byte of a FIXUPP packet */
    int             kind;
    int             thread;
    int             locat;
    int             relate;
