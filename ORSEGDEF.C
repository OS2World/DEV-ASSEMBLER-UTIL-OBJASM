    ...
    int     code;
{
    ...
    insert( (char *)segment_rec, segment_tree, seg_compare );
}
...
printf("Length of segment = %04X\n",length );
    ...
    segment = (NAME_T *)find( (char *)&search, name_tree,
                                                    name_compare, NULL );
    if ( segment == NULL ) fmt_error( "Undefined segment name" );
    name_idx = search.index;

    get_index( &search.index );
    class = (NAME_T *)find( (char *)&search, name_tree,
                                                    name_compare, NULL );
    if ( class == NULL ) fmt_error( "Undefined class name" );

    get_index( &search.index );
    overlay = (NAME_T *)find( (char *)&search, name_tree,
                                                    name_compare, NULL );
