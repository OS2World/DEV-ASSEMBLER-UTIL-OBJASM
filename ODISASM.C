...
int check_getc( unsigned long * );
int check_getw( unsigned long * );
int check_getl( unsigned long * );
int check_getv( unsigned long *, int );
int get_checkc( unsigned long * );
void instr_opcode( char * );
void instr_operand( char * );
int check_forward( INST_T [], int );
int mod0( int, int, char *, char *, unsigned char, int, int * );
int mod1( int, int, char *, char *, unsigned char );
int mod2( int, int, char *, char *, unsigned char, int, int * );
int mod3( int, char *, unsigned char, int, int * );
int do_sib( char *, unsigned char * );
int do_mod_rm( char *, unsigned char, unsigned char, int, int, int );
    ...
    unsigned long   *result;
{
    ...
    *result = (unsigned long)((unsigned char)ch);

    ch = buff_getc();
    if ( ch == EOF ) {
        return( TRUE );
    }
    *result += (unsigned long)((unsigned char)ch) << 8;

    ch = buff_getc();
    if ( ch == EOF ) {
        return( TRUE );
    }
    *result += (unsigned long)((unsigned char)ch) << 16;
    ch = buff_getc();
    if ( ch == EOF ) {
        return( TRUE );
    }
    *result += (unsigned long)((unsigned char)ch) << 24;
    ...
    int             class;
{
    ...
    text;               /* Prevent unused variable warnings */
    ...
    int             class;
{
    ...
    byte;                   /* Prevent unused varaible warnings */
}
    ...
    int             class;
{
    ...
    if ( offset == 0xFFFFFFFF ) {       /* This would generate a label in  */
    ...
    int             class;
{
    ...
    switch( data_check( 1 ) ) {
        ...
        case NORMAL:
            ...
            */
            if ( offset < 0x80 || offset >= 0xFFFFFF7F ) {
                bad_size = TRUE;
                short_text = "short ";
            } else {
                short_text = "";
    ...
    unsigned char   *base;
{
    ...
    switch( data_check(2) ) {
        ...
        case NORMAL:
            ...
            *base = (unsigned char)(sib_byte & 0x07L);
    ...
    int             class;
{
    ...
    mod = (unsigned char)(((int)mod_reg & 0xC0) >> 6);
    reg = (unsigned char)(((int)mod_reg & 0x38) >> 3);
    r_m = (unsigned char)(((int)mod_reg & 0x07) >> 0);
    ...
    int             class;
{
    ...
    unsigned char   mod;
    unsigned char   r_m;
    unsigned char   lll;        /* Described as LLL in 80386 Prog Ref Man. */
    unsigned char   ttt;        /* Described as TTT in 80386 Prog Ref Man. */
    unsigned char   esc_byte;
    unsigned char   mf;
    int             additional;
    int             size_needed;
    char            line[50];
    ...
    mod = (unsigned char)(((int)mod_reg & 0xC0) >> 6);
    lll = (unsigned char)(((int)mod_reg & 0x38) >> 3);
    r_m = (unsigned char)(((int)mod_reg & 0x07) >> 0);

    ttt = (unsigned char)(byte & 0x07);
    esc_byte = (unsigned char)(ttt << 3) + lll;

    if ( mod == 0x03 ) {
        esc_special( temp, line, esc_byte, r_m );
        additional = 0;
    } else {
        mf = (unsigned char)((esc_byte & 0x30) >> 4);
